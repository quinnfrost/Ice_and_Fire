...
public class DragonFlightUtils {
    ...
    public static boolean isBlockPassable(Level world, Heightmap.Types heightmapType, BlockPos blockPos) {
        return !heightmapType.isOpaque().test(world.getBlockState(blockPos));
    }

    public static BlockPos getBlockUp(Level world, Heightmap.Types heightmapType, BlockPos blockPos) {
        while (isBlockPassable(world, heightmapType, blockPos) && blockPos.getY() < world.getMaxBuildHeight()) {
    ...
    public static BlockPos getBlockUnder(Level world, Heightmap.Types heightmapType, BlockPos blockPos) {
        while (isBlockPassable(world, heightmapType, blockPos) && blockPos.getY() > 0) {
    ...
    public static Vec3 getGround(Level world, Vec3 vec3) {
        return new Vec3(
                vec3.x,
                getBlockUnder(world, Heightmap.Types.MOTION_BLOCKING, new BlockPos(vec3)).above().getY(),
                vec3.z
        );
    }
